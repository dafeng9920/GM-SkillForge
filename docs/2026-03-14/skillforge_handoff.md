...
## 17.5 前端交互骨架补充（新增且必须并入总纲）
> 这一节是对本交接文档的重要补充。  
> 上文主要定义了 SkillForge 的治理内核、合同骨架、8 Gate、AuditPack、n8n 分工和总体路线图。  
> 但在后续推进中，前端已经进一步收敛出一套新的 **交互主骨架**。  
> 它不是偏离总纲的新路线，而是 **这套治理内核在前端的交互实现层**。
...
**统一输入 -> 状态机裁决 -> orchestration -> 共享画布承接**

这条链是治理内核在交互侧的唯一正确实现方向。

### 17.5.2 三层交互结构

| 层 | 职责 | 说明 |
|---|---|---|
| 输入层 | 接住所有用户输入、补充说明、外部来源、继续追问 | 用户控制权始终保留在这里 |
| 状态机中间层 | 对意图做裁决、澄清、路径确认、后端能力分派 | 这是“宪法裁决能力”的交互执行器 |
| 画布层 | 展示当前状态、当前产物、下一步动作、证据与承接结果 | 画布不直接猜输入，只承接状态机输出 |

### 17.5.3 最关键的一句

**不是“输入框驱动画布”，而是“输入框驱动状态机，状态机驱动画布”。**
...
因为 SkillForge 要同时区别于两类系统：

1. **区别于普通 LLM 对话系统**
   - 不是开放式聊天
   - 不是 embedding / 向量直接拍脑袋选路径
   - 不是“模型说了算”

2. **区别于传统网页后台**
   - 不是每走一步都跳新页面、让用户重新理解页面
   - 不是首页有交互，后续全是单向输出
...
页面不再是“用户被路由到哪里”，而是“当前治理画布处于什么状态”。
...
- `Vetting / Audit / Permit / Dashboard`
  这些页面/区域，本质上都应被视为 **治理画布状态** 的承接面
  而不是彼此孤立的传统页面。

### 17.5.6 Dashboard 的重新定义

`Dashboard` 不再适合作为所有请求的默认中转页。
...
只有低置信输入或显式总览意图，才进入 `Dashboard / Overview`。

### 17.5.7 这一层与上文总纲的关系

必须明确：

- 上文的 `Intent Catalog + Orchestration + Skills`
  是 **治理内核与工程骨架**
- 这里新增的
  `统一输入 -> 状态机 -> orchestration -> 共享画布`
  是 **治理内核在前端的交互承接层**
...
## 17.6 Owner Review Layer / 二层报告制补充（新增且必须并入总纲）
> 这一节补的是“审计结果如何真正变成可裁决结果”。  
> 仅有技术 finding、trace、日志和证据，并不等于 owner 能做判断。  
> SkillForge 不能只会产出技术报告，还必须再多产出一层：
>
> **Owner Review Layer（老板可判定层 / 非技术裁决层）**

### 17.6.1 为什么这一层是必须品

如果系统只有技术层结果，而没有面向 owner 的裁决解释层，会立刻出现三类问题：

- 技术报告直接交给 owner，看不懂，无法验收
- 系统把“异常现象”直接写成“真实缺陷”，owner 无法识别过度结论
- 不同 finding 之间没有统一裁决格式，无法横向比较和汇总判断

所以 SkillForge 的主链不能停在：

**发现 -> 技术报告**

而必须继续到：

**发现 -> 裁决 -> Owner Review -> 交付**

### 17.6.2 二层报告制（必须锁死）

SkillForge 的报告体系建议固定为两层：
...
专门给 owner / 负责人 / 非技术裁决者使用。  
它不再堆技术细节，而是把 finding 压成固定裁决卡。

### 17.6.3 裁决卡固定 7 字段（建议作为标准结构）

每个 finding 都应至少输出以下 7 个字段：
...
5. **主要依据**
   - 只用 1–3 句人话解释为什么系统倾向当前判断

6. **最大疑点**
   - 明确写出为什么现在还不能 100% 下结论
...
这一层不是 UI 装饰，不是报告摘要页，而是治理内核的正式组成部分。

它应位于：

**Technical Evidence -> Finding Adjudicator -> Owner Review Builder -> AuditPack / Permit / 后续动作**

也就是说：

- Technical layer 负责“发现与举证”
- Adjudication layer 负责“判断真假、强度、影响、疑点”
- Owner Review layer 负责“让负责人能真正下决定”
...
#### B. Owner Review Builder

负责把 adjudication 结果组装成：
...
整份审计结果首页，不应先展示技术摘要，而应先展示 owner summary：
