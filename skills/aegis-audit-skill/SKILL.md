...
3. **Skill Vetter Protection**: 专用安全审计门禁（Inspired by ClawHub）：
   - 检测 `sudo`, `base64 --decode`, `curl | bash` 等提权/下载执行模式。
   - 识别过度权限请求（如：尝试读取 `.ssh`, `.gnupg`, 浏览器 cookies）。
   - 匹配恶意网络外连模式。
4. **Bloat Detection**: 自动识别并建议删除低质量、冗余的 AI 生成文本（针对“脱水策略”）。
