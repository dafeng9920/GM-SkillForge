...
class FastQuantTradingSystem:
    ...
    def load_historical_data(self, days: int = 120):
        ...
        # 使用并发加载
        hist_data = self.data_fetcher.get_historical_data_fast(
            self.symbols,
            days=days
        )
    ...
    def _execute_buy(self, symbol: str, signal: TradingSignal, account: dict, positions: dict):
        ...
        print(f"     ════════════════════════════════════════")
        print(f"     因子分析:")
        print(f"       动量: {signal.factors['momentum']:+.2f}")
        print(f"       波动: {signal.factors['volatility']:+.2f}")
        print(f"       成交: {signal.factors['volume']:+.2f}")
        print(f"       RSI:  {signal.factors['rsi']:+.2f}")
        print(f"     ════════════════════════════════════════")
    ...
    def _execute_sell(self, symbol: str, signal: TradingSignal, positions: dict):
        ...
        if order.status == OrderStatus.FILLED:
            print(f"  卖出: {symbol} {position.quantity}股 @ ¥{order.filled_price:.2f} ({reason})")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='A股量化交易系统 - 高性能版')
